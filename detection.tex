...
Throughput the section, we assume that nodes authenticate their messages using a private signature key. 
% Hatem: trim
...
% % \mn[Stef]{how can it be `only'? it has to get the chunk for the first time at some point}
...
Third, the initiator $b$ decides based on the received responses a complaint should be filed to the source.
...
% \mn[Stef]{unclear: do they also forward to neighbors? probably not}
