...
% 
% Note that in case $b$ decides to fire a complaint to the source, according to the formulas below, it includes the IDs of all the other participating peers in the complaint whose satisfaction level is below $\sigma$.
% Hence, for a 
% 
% In addition, in case the source detects
% In order to
% To that end, we discuss in  the constraints that prevent malicious peers from abusing the drop detection mechanism by sending a complaint to falsely convince the source to take a counter action against benign peers.
% Moreover,
% 
% In order to minimize the likeliness that malicious peers can abuse the drop detection mechanism, i.e., malicious peers can generate a complaint to the source 
